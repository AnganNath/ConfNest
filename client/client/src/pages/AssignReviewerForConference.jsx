...
export default function AssignReviewerForConference() {
  const { id } = useParams()
  ...
  async function assign(subId){
    ...
    if(!reviewerId) return alert("Select reviewer first")
      ...
      {subs.map(s =>
        <Card key={s._id}>
          <CardContent>
            ...
            <Button variant="contained" onClick={()=>assign(s._id)}>Assign</Button>
          </CardContent>
        </Card>
      )}
    </div>
